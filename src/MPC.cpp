...
class FG_eval {
 public:
  ...
  AD<double> polyeval(Eigen::VectorXd coeffs, const AD<double> x) {
  ...
  AD<double> polyderivative(Eigen::VectorXd coeffs, const AD<double> x) {
    AD<double> result = 0.0;
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (t = 0; t < N - 1; t++) {
      fg[0] += 16000 * CppAD::pow(vars[delta_start + t], 2);
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // The upper and lower limits of delta are set to -1.0 and 1.0
  // (values in radians).
